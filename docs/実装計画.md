...
- **特に重要な点**: iOS Safariでの安定動作（LINE SDKを使用したカスタム認証フロー）
- **開発状況**: ディレクトリ構造のみ作成済み、これから新規実装
- **環境設定**: Firebase/LINE Developersはこれから設定予定
...
#### 2.3 API エンドポイントの実装
- `/api/line-callback`: LINE OAuth コールバック処理
- `/api/auth/custom-token`: カスタムトークン生成API
...
### LINE SDKを使用したカスタム認証フロー
1. フロントエンドでLINEログインボタンを実装
2. ユーザーがボタンをクリックするとLINE認証画面へリダイレクト
3. 認証後、コールバックURLにリダイレクト（認可コード付き）
4. バックエンドで認可コードを使ってアクセストークンを取得
5. アクセストークンを使ってユーザープロフィールを取得
6. ユーザー情報を基にFirebase Admin SDKでカスタムトークンを生成
7. フロントエンドにカスタムトークンを返却
8. フロントエンドでカスタムトークンを使ってFirebase Authenticationにサインイン

### iOS Safariでの問題回避策
- LINE SDKを使用することで、リダイレクト時のセッション共有問題を回避
...
if (process.env.NODE_ENV !== 'production') {
  ...
  connectFirebaseEmulator(functions, 'localhost', 5001);
}
```
